...
import {
  ...
  api_media_insert_input,
  api_media_update_column,
...
import { BubbleSettings } from "./components/Dialog/BubbleComponent";
import { BubbleItemSettings } from "./components/Dialog/BubbleItemComponent";
...
export interface IMedia
  extends Omit<getSelectedComponent_component_media, "__typename" | "id"> {
  fk_component_id?: string;
  id?: string;
}
...
const Settings = ({ component, languages, loading }: ISettingsProps) => {
  ...
  const handleOnSaveClick = () => {
    if (!form || !form.current || form.current.handleSubmit(undefined)) {
      console.warn(
        "This component's setting widget is missing a Formik form implementation which handles the validation and submission! Did you maybe forget to set the 'ref' prop on the form?"
      );
    } else {
  ...
  const handleOnSubmit = (config: SubmitConfig) => {
    ...
    let deleteMediaIds: string[] =
      (media && media.delete.filter((m) => !!m.id).map((m) => m.id!)) || [];
    ...
    // media
    const _media = (media && media.upsert) || [];
    const upsertableMediaData: api_media_insert_input[] = [];
    ...
    for (let i = 0; i < _texts.length; i++) {
      ...
      _text.translations.forEach((t) => {
        ...
        if (language) {
          ...
          console.error(
            "Error! No language for this translation found in cache!"
          );
        }
      });
    ...
    // Loop through each media (usually anyway just one), and prepare the nested objects for the mutation
    for (let i = 0; i < _media.length; i++) {
      const _m = cloneDeep(_media[i]);
      upsertableMediaData.push({
        id: _m.id,
        fk_component_id: component && component.id,
        type: _m.type,
        url: _m.url,
    ...
    updateComponent({
      variables: {
        ...
        textUpdateColumns: [
          api_text_update_column.translatable,
          api_text_update_column.placeholder,
        ],
        ...
        mediaData: upsertableMediaData,
        mediaUpdateColumns: [
          api_media_update_column.type,
          api_media_update_column.url,
        ],
        deleteTextIds: [],
      ...
      <Grid container className={classes.container} direction="row">
        <Grid item container xs={12} direction="row" justify="space-between">
          ...
          <Grid item xs={3} container alignItems="center" justify="flex-end">
            ...
            {component ? <ActionMenu data={component} /> : null}
          </Grid>
        </Grid>
